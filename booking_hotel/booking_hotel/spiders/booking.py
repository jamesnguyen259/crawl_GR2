...
class BookingSpider(scrapy.Spider):
    ...
    start_urls = ['https://www.booking.com/searchresults.vi.html?dest_id=6228;dest_type=region;ss=H%C3%A0%2BN%E1%BB%99i%2C%2BVi%E1%BB%87t%2BNam']
    ...
    def parse_item(self, response):
        ...
        #get rating
        try:
            temp = response.css("span.hp__hotel_ratings__stars.nowrap i.bk-icon-wrapper.bk-icon-stars.star_track::attr(title)").extract_first().encode("utf-8")
            if temp == "1 sao":
                item['rating'] = "1"
            elif temp == "2 sao" or temp == "Khách sạn 2 sao":
                item['rating'] = "2"
            elif temp == "Khách sạn 3 sao":
                item['rating'] = "3"
            elif temp == "Khách sạn 4 sao":
                item['rating'] = "4"
            else:
                item['rating'] = "5"
        except:
