    ...
    def parse_link(self, response):
        ...
        if 'Chủ đề' in extract_with_css('div.junkie-alert.yellow p span strong::text'):
          yield{
            ...
            'event_time': response.css('div.junkie-alert.yellow p span::text').extract()[1].encode('utf-8').strip(),
            'event_place': response.css('div.junkie-alert.yellow p span::text').extract()[2].encode('utf-8').strip()
          }
        else:
          yield {
            'event_name': extract_with_css('div.junkie-alert.yellow h2::text'),
            'event_time': extract_with_css('div.junkie-alert.yellow p span::text'),
            'event_place': response.css('div.junkie-alert.yellow p span::text').extract()[1].encode('utf-8').strip()
          }
